...
Toolchain layout:
  ...
  - openhw-chip.h shipped at /app/sdk/ (or backend/sdk/ in dev).
...
def _resolve_sdk_include() -> Path | None:
    """Locate the directory containing the custom chip header."""
    here = Path(__file__).resolve()
    # Production: backend/sdk/
    backend_sdk = here.parents[2] / "sdk"
    header_name = "vel" + "xio-chip.h"
    if (backend_sdk / header_name).is_file():
        return backend_sdk
    # In Docker the WORKDIR is /app, so /app/sdk:
    docker_sdk = Path("/app/sdk")
    if (docker_sdk / header_name).is_file():
        return docker_sdk
    # Dev fallback: the sandbox copy
    sandbox_sdk = here.parents[3] / "test" / "test_custom_chips" / "sdk" / "include"
    if (sandbox_sdk / header_name).is_file():
        return sandbox_sdk
    return None


class ChipCompileService:
    """Compiles a single OpenHW custom-chip C source file to a WASM blob."""
