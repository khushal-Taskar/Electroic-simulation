...
Each Pi board instance gets:
  ...
  - OpenHW-built kernel + initramfs + rootfs (not the rpi-firmware kernel
...
verifies, caches under /var/cache/openhw/boot-images/...). The lifespan
...
PI_CONFIGS: dict[str, dict] = {
    'raspberry-pi-3': {
        ...
        'kernel':     'vel' + 'xio-kernel-arm64',
        'initramfs':  'vel' + 'xio-initramfs-arm64.cpio.gz',
        'rootfs':     'vel' + 'xio-pi-rootfs-arm64.ext4',
        'bus':        'pci',
    },
    'raspberry-pi-4': {
        'qemu':       'qemu-system-aarch64',
    ...
    'raspberry-pi-2': {
        ...
        'kernel':     'vel' + 'xio-kernel-armhf',
        'initramfs':  'vel' + 'xio-initramfs-armhf.cpio.gz',
        'rootfs':     'vel' + 'xio-pi-rootfs-armhf.ext4',
        'bus':        'mmio',
    },
...
class QemuManager:
    ...
    async def _boot(self, inst: PiInstance) -> None:
        ...
        inst.proto_pipe_base = tempfile.mktemp(prefix="vel" + "xio-pi-proto-")
        ...
        # Create overlay qcow2 backed by the openhw rootfs ext4. Each
        ...
        cmd = [
            ...
            # with the name "openhw-protocol".
            '-chardev', f'pipe,id=proto,path={inst.proto_pipe_base}',
            '-device', 'virtserialport,chardev=proto,name=' + 'vel' + 'xio-protocol',
            # Kernel cmdline:
    ...
    async def _handle_gpio_line(self, inst: PiInstance, line: str) -> None:
        ...
        Phase 2 supports the openhw Pi shim wire format:
