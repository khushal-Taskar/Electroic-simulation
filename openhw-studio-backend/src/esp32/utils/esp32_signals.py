...
to drive the pin. The constants below name the signals that openhw
...
def ledc_signal_for_channel(channel: int) -> int:
    """Map an openhw-style unified LEDC channel index (0..15) to its
    GPIO Matrix signal source id.

    The ESP32 LEDC hardware has two channel groups: 8 high-speed (HS)
    and 8 low-speed (LS). openhw unifies them into a single 0..15
